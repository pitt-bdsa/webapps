...
function elementToFeature(element){
    function mapElementToGeometryType(e){
        ...
        if(e.type == 'polyline' && e.closed == true){
            ...
            g.coordinates=e.center.slice(0,2);
            g.properties.majorRadius = g.properties.minorRadius = e.radius;
...
function featureToElement(feature){
    ...
    let e = {
        ...
        lineWidth:Math.max(0.0001, parseFloat(p.rescale && p.rescale.strokeWidth ? p.rescale.strokeWidth : p.strokeWidth)),//convert zero to very small number
    };
    ...
    if(g.type=='MultiLineString'){
        ...
        e = g.coordinates.map((c,index)=>{
            ...
            singleLineString.user = Object.assign({},singleLineString.user,{MultiLineString:multiLineStringID});
